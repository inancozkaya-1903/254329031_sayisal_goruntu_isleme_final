...
# =========================
# ✅ 3.1 hücresi için gerekli çıktı: median_imgs (len=9)
# =========================
median_imgs = random9_train_df["img_median"].tolist()[:9]
if len(median_imgs) != 9:
    raise ValueError(f"median_imgs uzunluğu {len(median_imgs)}. 3.1 hücresi 9 örnek bekliyor.")
...
# Output: Her yöntem için eşik değerleri tablosu (mask_df)
# =========================

# ---- Guard (pipeline: median_imgs VAR) ----
...
if "label" not in random9_train_df.columns:
    raise KeyError("random9_train_df içinde 'label' kolonu yok. Pipeline kolon isimleri değişmiş olabilir.")
if len(median_imgs) != 9:
...
for idx in range(9):
    img = median_imgs[idx]
    label = random9_train_df.iloc[idx]["label"]

    # None / invalid kontrol
...
sk_cols = [c for c in mask_df.columns if c.startswith("sk_")]
mask_df = mask_df[base_cols + sorted(sk_cols)]

display(mask_df)

# =========================
# ✅ Pipeline helper: make_binary_mask (sonraki hücre hata vermesin diye)
# =========================
...
##4.3 Segmentation Overlay (RGB + ROI Mask)
"""
...
# =========================
# 4.3 RGB Overlay – Nihai Segmentasyon Maskesinin Görselleştirilmesi
# =========================

# ---------- Guard ----------
if "random9_train_df" not in globals():
    raise NameError("random9_train_df bulunamadı.")
if "selected_masks" not in globals():
    raise NameError("selected_masks bulunamadı. (CCL sonrası nihai ROI maskeleri)")
if len(selected_masks) != 9:
    raise ValueError(f"selected_masks len=9 olmalı. Şu an: {len(selected_masks)}")

for col in ["img_rgb", "img_gray"]:
    if col not in random9_train_df.columns:
        raise KeyError(f"random9_train_df içinde '{col}' yok. Mevcut kolonlar: {list(random9_train_df.columns)}")

labels = random9_train_df["label"].tolist() if "label" in random9_train_df.columns else [""] * 9


# ---------- Crop bbox hesaplayan yardımcı (senin crop mantığınla uyumlu) ----------
def _roi_bbox(gray_crop, dark_q=0.30):
    thr = np.quantile(gray_crop, dark_q)
    mask = (gray_crop <= thr).astype(np.uint8)
    ys, xs = np.where(mask > 0)
...
def crop_border_safe_with_bbox(gray,
                               base_trim=0.045,
                               min_trim=0.015,
                               step=0.005,
                               gap_px=22,
                               dark_q=0.30):
    """
    Senin crop_border_safe ile aynı karar mekanizması,
    ek olarak final crop bbox'u (x1,y1,x2,y2) döndürür.
    bbox, ORIJINAL görüntü koordinatındadır.
    """
    if gray is None:
        return None, None
...
# ---------- Overlay helpers ----------
def ensure_rgb(img):
    """BGR/RGB karışıklığı olmasın diye: img zaten RGB ise dokunmaz, BGR ise sen bilirsin.
    Burada sadece 3 kanal garanti ediyoruz."""
    if img.ndim == 2:
        return np.stack([img, img, img], axis=-1)
    if img.shape[-1] == 3:
        return img
    raise ValueError("img_rgb 3 kanallı olmalı.")

def overlay_mask_on_rgb(rgb, mask_u8, color=(0, 255, 0), alpha=0.35):
    """
    rgb: uint8 [H,W,3]
    mask_u8: uint8 [H,W] 0/255
    color: overlay rengi (R,G,B)
    """
    ...
    out[m] = (1 - alpha) * out[m] + alpha * color_layer[m]
    return np.clip(out, 0, 255).astype(np.uint8)


# ---------- Build full-size masks + overlay ----------
overlay_imgs = []
full_masks = []

# crop parametreleri SENİN KULLANDIĞIN ile aynı:
...
for i in range(9):
    rgb = ensure_rgb(random9_train_df.iloc[i]["img_rgb"]).astype(np.uint8)
    gray = random9_train_df.iloc[i]["img_gray"].astype(np.uint8)

    _, (x1, y1, x2, y2) = crop_border_safe_with_bbox(
        gray,
    ...
    # selected_mask crop uzayında (y2-y1, x2-x1) boyutunda olmalı
    sel = (selected_masks[i] > 0).astype(np.uint8) * 255

    # Boyut kontrol: mismatch varsa (nadir), güvenli resize (en yakın komşu)
    ch, cw = (y2 - y1), (x2 - x1)
    if sel.shape[0] != ch or sel.shape[1] != cw:
        sel = cv2.resize(sel, (cw, ch), interpolation=cv2.INTER_NEAREST)

    full = np.zeros((rgb.shape[0], rgb.shape[1]), dtype=np.uint8)
    full[y1:y2, x1:x2] = sel
    full_masks.append(full)

    ov = overlay_mask_on_rgb(rgb, full, color=(0, 255, 0), alpha=0.35)
    overlay_imgs.append(ov)
...
for i in range(9):
    ...
    ax.set_title(f"Ö{i+1} | RGB Overlay\n{labels[i]}", fontsize=9, pad=2)

plt.tight_layout(pad=0.8)
plt.show()

print("✅ RGB overlay üretildi (orijinal RGB üzerinde).")
print("Not: Maske crop uzayından bbox ile orijinale geri yerleştirildi.")
...
"""## 5.5 Feature Tablosu ve Çıktıların Yorumlanması"""
